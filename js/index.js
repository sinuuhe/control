...
firebase.auth().onAuthStateChanged(firebaseUser => {
    ...
    if (!firebaseUser) {
        ...
        cu = database.ref('users/' + userId).on('value', function (s) {
            if (s.val().type == "NORMAL") window.location.pathname = "Users/sinuhe/Documents/controlActivos/indexn.html";
            if (s.val().type == "ADMINISTRADOR") window.location.pathname = "Users/sinuhe/Documents/controlActivos/index.html";
            this.cu = s.val();
        })
...
function newTemporalKeeperSearch(temporalActiveInput) {
    ...
    //here is where we filter
    buildTableTmp(activesArray, 'newTemporalKeeperResultsTable', this.activeFieldsTmp, true);
}
...
function unSelectTempActive(activeId) {
    if (this.currentQueryResult[activeId] != undefined)
        this.currentQueryResult[activeId].quantity += this.tmpActiveList[activeId].quantity;
    else
    ...
    buildTableTmp(Object.values(this.currentQueryResult), 'newTemporalKeeperResultsTable', this.activeFieldsTmp, true);
...
function confirmTempKeeper(activeId) {
    if (document.getElementById('activeQuantityTmp').value > this.selectedActive.quantity) {
        ...
        if (document.getElementById('activeQuantityTmp').value == this.selectedActive.quantity) {
            delete this.currentQueryResult[this.selectedActive.id];
        }
        if (document.getElementById('activeQuantityTmp').value < this.selectedActive.quantity) {
            this.currentQueryResult[this.selectedActive.id].quantity = this.currentQueryResult[this.selectedActive.id].quantity - document.getElementById('activeQuantityTmp').value;
            this.selectedActive.quantity = document.getElementById('activeQuantityTmp').value;
        }
        this.temporalList.push(this.selectedActive);
        this.tmpActiveList[this.selectedActive.id] = this.selectedActive;
        buildTableTmp(Object.values(this.currentQueryResult), 'newTemporalKeeperResultsTable', this.activeFieldsTmp, true);
        buildTableTmp(this.temporalList, 'selectedTemporalResultsTable', this.activeFieldsTmp, false);
    }
}
