...
var activesCategories = [
    {
        propertie: 'Herramientas',
        value: "HERRAMIENTAS"
    },
    {
        propertie: 'Máquinas Eléctricas',
        value: "MÁQUINAS ELÉCTRICAS"
    },
    {
        propertie: 'Maquinaria Pesada',
        value: "MAQUINARIA PESADA"
    },
    {
        propertie: 'Herramienta Especializada',
        value: "HERRAMIENTA ESPECIALIZADA"
    },
    {
        propertie: 'Equipo de Cómputo',
        value: "EQUIPO DE CÓMPUTO"
    },
    {
        propertie: 'Equipo de Oficina',
        value: "EQUIPO DE OFICINA"
    },
    {
        propertie: 'Mobiliario',
        value: "MOBILIARIO"
    }
];
$(document).ready(function () {
    ...
    $('#secondActiveFilterSelect').on('change', function (e) {
        ...
        var select = document.getElementById('secondActiveFilterSelect').value;
...
function finishTemporalKeeper(tmpKeeperName, tmpReceptorName, tableId) {
    ...
    var promise = database.ref('actives/temporalKeeper').push({
        outDate: getCurrentDate(true),
        returnDate: document.getElementById('tmpReturnDate').value,
        ...
        requester: tmpKeeperName,
        receptor: tmpReceptorName,
        user: this.cu,
        status: 'current'
    });
...
function moveActiveStatus(index) {
    if (index < this.temporalList.length) {
        ...
        prom.then(function (r) {//updating on ACTIVO
            if (this.currentQueryResult[this.temporalList[index].id] == undefined)
                return database.ref('actives/status/ACTIVO/' + this.temporalList[index].id).set(null)
            else {
                return database.ref('actives/status/ACTIVO/' + this.temporalList[index].id).update({
                    quantity: this.currentQueryResult[this.temporalList[index].id].quantity
                })
            }
        }, function (e) { })
            .then(function (r) {
                return database.ref('actives/brand/' + this.temporalList[index].brand + '/' + this.temporalList[index].id).update({
                    status: 'TEMPORAL',
                    quantity: this.temporalList[index].quantity
                })
            })
            .then(function (r) {
...
function registerActive() {
    ...
    var category = document.getElementById('selectedActiveCategory').value;
    var totalQuantity = document.getElementById('activeQuantity').value;
    var availableQuantity = totalQuantity;
    ...
    if (serialNumber.toUpperCase() == "NA" || serialNumber == "" || serialNumber == " ") {
...
function loadBuildings(elementId, nextElementId, selectedRoomInput, selectedBuildingInput, otherFlah) {
    ...
    building.on('value', function (snapshot) {
        ...
        for (var element of buildingArray) {
            if (!(element.name == "Resguardo Personal" && elementId == "buildingListEmploye")) {
                var listItem = document.createElement('li');
                var option = document.createElement('a');
                option.value = element.name;
                option.text = element.name;
...
function fillSearchInput(HTMLElementId, filters, propertie) {
    var _filters = this[filters];
    _filters[propertie] = document.getElementById(HTMLElementId).value.toUpperCase();
};
...
function getCurrentDate(rSpanishDate) {
    var today = new Date();
    var day = today.getDate();
    var month = today.getMonth(); //January is 0!
    var year = today.getFullYear();
...
function temporalKeeperQuery(hideElement, classToSet, showElement) {
    document.getElementById(hideElement).classList.add(classToSet);
    document.getElementById(showElement).classList.remove(classToSet);
...
function fillOption(inputId, path, local, propList, filter) {

    if (!local) {
        console.log('path ' + path)
        var query = database.ref(path);
        query.on('value', function (s) {
            var result = s.val();
...
function makeTable(filteredResults, tableFields, tableId, firstInput, secondInput, firstSelect, secondSelect) {
    if (filteredResults.length <= 0) {
        ...
        var resultsTable = document.getElementById(tableId);
        resultsTable.innerHTML = "";

        var _tr = document.createElement('tr');
        _tr.className = "red darken-4 white-text";
        for (field of tableFields) {
            var title = document.createElement('th');
            title.innerText = field.title
            _tr.appendChild(title);
        }

        resultsTable.appendChild(_tr);

        for (var active of filteredResults) {
            var newRow = document.createElement('tr');
            //fill data
            for (var field of tableFields) {
                var td = document.createElement('td');
                td.innerText = active[field.propertie];
